    ...
    <div className="space-y-6">
      <section className="rounded-2xl border border-cyan-500/20 bg-slate-900/70 p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-[0.22em] text-cyan-300">Task details</p>
            <h1 className="text-2xl font-bold text-white">{task?.title}</h1>
            <p className="mt-1 text-sm text-slate-400">Project: {project?.name ?? 'Unknown'}</p>
          </div>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <SectionCard title="Comments">
          {canComment ? (
            ...
            <p className="mb-3 text-sm text-slate-400">Your role cannot post comments.</p>
          )}
          ...
          {(commentsQuery.data ?? []).length === 0 ? (
            ...
            <ul className="space-y-2">
              {(commentsQuery.data ?? []).map((comment) => (
                <li key={comment.id} className="rounded-xl border border-slate-700 bg-slate-950/40 p-3">
                  <p className="text-sm text-slate-100">{comment.content}</p>
                  <p className="mt-1 text-xs text-slate-400">{comment.user.fullName}</p>
          ...
          {(attachmentsQuery.data ?? []).length === 0 ? (
            ...
            <ul className="space-y-2">
              {(attachmentsQuery.data ?? []).map((attachment) => (
                <li key={attachment.id} className="rounded-xl border border-slate-700 bg-slate-950/40 p-3">
                  <p className="text-sm font-semibold text-slate-100">{attachment.fileName}</p>
                  <p className="mt-1 text-xs text-slate-400">{bytesToSize(attachment.fileSize)}</p>
